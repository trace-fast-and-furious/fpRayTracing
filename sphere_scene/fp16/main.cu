    ...
    curandState *rand_state) 
{
    ...
    d_image[global_index] = (unsigned char)(__half2float(255.99f * pixel_color[0]));
    d_image[global_index + 1] = (unsigned char)(__half2float(255.99f * pixel_color[1]));
    d_image[global_index + 2] = (unsigned char)(__half2float(255.99f * pixel_color[2]));
}
...
int main() {
	...
	// creating directory
	string directory = "../../img/fp16";
	if (mkdir(directory.c_str(), 0777) == -1)
		cerr << "Error :  " << strerror(errno) << endl;

	string img_path = directory + "/" + to_string(timeStamp) + "_" + to_string(image_width) + "_" + to_string(samples_per_pixel) + "_" + to_string(max_depth) + "fp16_img.ppm";
