...
__device__ fp_custom schlick(fp_orig cosine_f, fp_orig ref_idx_f)
{
...
__device__ fp_custom schlick(fp_custom cosine, fp_custom ref_idx)
{
    fp_custom r0 = (1.0f - ref_idx) / (1.0f + ref_idx);
    r0 = r0 * r0;
    return r0 + (1.0f - r0) * pow((1.0f - cosine), 5.0f);
...
__device__ bool refract(const vec3 &v, const vec3 &n, fp_orig ni_over_nt_f, vec3 &refracted)
{
...
__device__ bool refract(const vec3 &v, const vec3 &n, fp_custom ni_over_nt, vec3 &refracted)
{
    vec3 uv = unit_vector(v);
    fp_custom dt = dot(uv, n);
    fp_custom discriminant = 1.0f - ni_over_nt * ni_over_nt * (1 - dt * dt);
...
{
public:
    __device__ metal(const vec3 &a, fp_orig __f) : albedo(a), fuzz(__f < 1.0f ? fp_orig_to_custom(__f) : fp_orig_to_custom(1)) {}
...
{
public:
    __device__ dielectric(fp_orig ri) : ref_idx(fp_orig_to_custom(ri)) {}
