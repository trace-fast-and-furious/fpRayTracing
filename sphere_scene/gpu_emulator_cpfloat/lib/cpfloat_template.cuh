...
#define INITBIT(seed) pcg32_srandom_r(seed, clock64(), (intptr_t)seed);
...
#define INITBIT(seed) *seed = clock64();
...
#define PRNG_BIT_INIT                                                        \
  ...
  {                                                                          \
    fpopts->BITSEED = (cpfloat_bitseed_t *)malloc(sizeof(*fpopts->BITSEED)); \
...
__host__ __device__ time_t time_c(time_t *t)
{
  ...
  time_t seconds_since_epoch = current_time / CLOCKS_PER_SEC;

  /* If the t argument is not NULL, store the current time in it */
  if (t != NULL)
  {
...
#define PRNG_RAND_INIT                                                          \
  if (fpopts->RANDSEED == NULL)                                                 \
  {                                                                             \
    fpopts->RANDSEED = (cpfloat_randseed_t *)malloc(sizeof(*fpopts->RANDSEED)); \
...
__host__ __device__ static inline int VALIDATE_INPUT(const optstruct *fpopts)
{
  ...
  {
    if (strcmp_c(fpopts->format, valid_formats[i]))
      retval = 0;
  }
                                                        ...
                                                        int *retval)
{
  ...
  *retval = 0;
  ...
  cpfloat_exponent_t emax = fpopts->explim == CPFLOAT_EXPRANGE_TARG ? fpopts->emax : DEFEMAX;
                                                  ...
                                                  LOCPARAMS *lparams)
{
  if (ABS(A) < params->xmin && params->emin > 1 - DEFEMAX)
  {
    ...
    leadmask ^= (((INTCONST(1) << (params->precision - lparams->precision)) - INTCONST(1))
...
#define RN_TIES_TO_AWAY_SCALAR_SAME_EXP(x, y, p)                  \
  ...
  {                                                               \
    ...
    *(x) = FPOF((INTOF(y) +                                       \
                 (INTCONST(1) << (DEFPREC - 1 - p->precision))) & \
                p->leadmask);

#define RN_TIES_TO_AWAY_SCALAR_OTHER_EXP(x, y, p, lp)              \
  if (ABS(y) < p->ftzthreshold)                                    \
  { /* Underflow */                                                \
  ...
  {                                                                \
    ...
    *(x) = FPOF((INTOF(y) +                                        \
                 (INTCONST(1) << (DEFPREC - 1 - lp->precision))) & \
                lp->leadmask);                                     \
  }
...
#define RN_TIES_TO_EVEN_SCALAR_OTHER_EXP(x, y, p, lp)                                                                                            \
  ...
  {                                                                                                                                              \
    UPDATE_LOCAL_PARAMS(y, p, lp);                                                                                                               \
    INTTYPE LSB = ((INTOF(y) >> (DEFPREC - lp->precision)) & INTCONST(1)) | (lp->precision == 1 && DEFEMAX != p->emax); /* Hidden bit is one. */ \
...
#define RD_TWD_ZERO_SCALAR_OTHER_EXP(x, y, p, lp)  \
  ...
  else if (ABS(y) > p->xmax && ABS(y) != INFINITY) \
  { /* Overflow */                                 \
...
#define RS_PROP_SCALAR(x, y, p, lp)                                                               \
  ...
  { /* Underflow */                                                                               \
    ...
    if (expx == -DEFEMAX)                                                                         \
    { /* No implicit bit (x is subnormal). */                                                     \
      expx += 1;                                                                                  \
    }                                                                                             \
    else                                                                                          \
    { /* Make implicit bit explicit (x is normal). */                                             \
      trailfrac = (INTOF(y) & (FULLMASK >> NLEADBITS)) | (INTCONST(1) << (DEFPREC - 1));          \
    }                                                                                             \
    int localemin = p->subnormal == CPFLOAT_SUBN_USE ? p->emin - (int)p->precision + 1 : p->emin; \
  ...
  {                                                                            \
    ...
    {                                                                          \
      ...
      {                                                                        \
        ...
        CONCATENATE(SCALARFUN, _SCALAR_SAME_EXP)                               \
        (X, Y, p)                                                              \
    ...
    {                                                                          \
      ...
      {                                                                        \
        DEPARENTHESIZE_MAYBE(PREPROC)                                          \
        CONCATENATE(SCALARFUN, _SCALAR_OTHER_EXP)                              \
        (X, Y, p, lp)                                                          \
            DEPARENTHESIZE_MAYBE(POSTPROC)                                     \
      }                                                                        \
    }                                                                          \
...
#define ADD_BITFLIP_CODE_INTRODUCE_BITFLIP(PARALLEL_TYPE, X)                     \
  ...
  {                                                                              \
    PRNG_RAND_INIT                                                               \
    size_t flippos;                                                              \
    const size_t totbits = fpopts->flip == CPFLOAT_FP_SOFTERR ? NBITS : DEFPREC; \
...
#define GENERATE_INTERFACE_SUBFUNCTIONS(FUNNAME,                                          \
  ...
  {                                                                                       \
    ...
    ADD_BITFLIP_CODE(BITFLIP_CODE)                                                        \
    (PARALLEL_TYPE, X) return retval;                                                     \
  }
...
#define GENERATE_INTERFACE(FUNNAME, FOUTPUT, FINPUT, X, Y,                                      \
  ...
  {                                                                                             \
    if (numelem < CONCATENATE(OPENMP_THRESHOLD_, FPTYPE))                                       \
      return GENERATE_SUBFUN_NAME(FUNNAME, PARALLEL_SUFFIX_SEQ)(DEPARENTHESIZE_MAYBE(FCALLOUT), \
                                                                DEPARENTHESIZE_MAYBE(FCALLIN),  \
                                                                numelem, fpopts);               \
    else                                                                                        \
      return GENERATE_SUBFUN_NAME(FUNNAME, PARALLEL_SUFFIX_PAR)(DEPARENTHESIZE_MAYBE(FCALLOUT), \
                                                                DEPARENTHESIZE_MAYBE(FCALLIN),  \
                                                                numelem, fpopts);               \
  }
#else /* #ifdef _OPENMP */
...
#define GENERATE_NEXTAFTER_INTERFACE(FUNNAME, TYPETO)                                      \
  ...
  {                                                                                        \
    if (numelem < CONCATENATE(OPENMP_THRESHOLD_, FPTYPE))                                  \
      return GENERATE_SUBFUN_NAME(FUNNAME, PARALLEL_SUFFIX_SEQ)(X, A, B, numelem, fpopts); \
    else                                                                                   \
      return GENERATE_SUBFUN_NAME(FUNNAME, PARALLEL_SUFFIX_PAR)(X, A, B, numelem, fpopts); \
  }
#else /* #ifdef _OPENMP */
#define GENERATE_NEXTAFTER_INTERFACE(FUNNAME, TYPETO)                                    \
  GENERATE_NEXTAFTER_SUBFUNCTIONS(FUNNAME, TYPETO, SEQ, PARALLEL_SUFFIX_SEQ)             \
...
__host__ __device__ static inline int CONCATENATE(ADDSUFFIXTO(cpfloat), _parallel)(FPTYPE *X,
                                                                          ...
                                                                          optstruct *fpopts)
{
  return GENERATE_SUBFUN_NAME(fpround, PARALLEL_SUFFIX_PAR)(X, A, numelem, fpopts);
}
#endif /* #ifdef _OPENMP */
...
GENERATE_INTERFACE(ldexp,
                   ...
                   X, (A, exp),
                   NOOP, INTRODUCE_BITFLIP,
                   *(X + i) = ADDSUFFIXTO(ldexp)(*(A + i), *(exp + i));
                   , NOOP)
...
GENERATE_INTERFACE(ilogb,
                   ...
                   NOOP, NO_BITFLIP,
                   FPTYPE loctmp;
                   , *(exp + i) = ADDSUFFIXTO(ilogb)(loctmp);)
...
#define FIND_ROUNDING_PRECISION(extra_instructions)                      \
  (                                                                      \
      extra_instructions int temp = ADDSUFFIXTO(ilogb)(*(A + i));        \
...
GENERATE_INTERFACE(fpclassify,
                   ...
                   ,
                   if (loctmp == 0) * (r + i) = FP_ZERO;
                   else if (ABS(&loctmp) < params.xmin) * (r + i) = FP_SUBNORMAL;
                   else if (ABS(&loctmp) < INFINITY) * (r + i) = FP_NORMAL;
                   else if (isinf(loctmp)) * (r + i) = FP_INFINITE;
                   else * (r + i) = FP_NAN;)
