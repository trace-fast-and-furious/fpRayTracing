	...
	curandState *rand_state)
{
	...
	*d_world = new hittable_list(d_list, (2 * n_obj) * (2 * n_obj) + 1 + 3);
	...
	vec3 lookat(0, 0, 0);
	fp_orig dist_to_focus = val((lookfrom - lookat).length()); // 10.0f
	fp_orig aperture = 0.1f;
	...
	curandState *local_rand_state)
{
	...
	for (int i = 0; i < depth; i++)
	{
		...
		else
		{
			...
			fp_orig t = 0.5 * (val(unit_direction.y()) + 1.0); // color gradation in y(vertical) direction
	...
	curandState *rand_state)
{
	...
	if ((i >= image_width) || (j >= image_height))
		return; // Check index out of range
	...
	d_image[global_index] = (unsigned char)(val(255.99f * pixel_color[0]));
	d_image[global_index + 1] = (unsigned char)(val(255.99f * pixel_color[1]));
	d_image[global_index + 2] = (unsigned char)(val(255.99f * pixel_color[2]));
}
...
int main()
{
	...
	string directory = "../images/gpu_emulator_cpfloat/" + string(typeid(fp_orig).name());
	if(DATE) directory += "_" +  to_string(timeStamp);
	...
	ppmSave(img_path.c_str(), h_image, image_width, image_height);
