...
void ppmSave(const char* filename, unsigned char* inData, int w, int h, int obj, int s)
{
    ...
    fclose(fp);
		...
		printf("\n(  Image Resolution   )  Picture Size: %d x %d (# of Pixels %d)\n", h, w, (h)*(w));
    printf("( Rendering Condition )  # of Objects: %d, # of samples per pixel: %d\n", obj, s);
}
