...
void ppmSave(const char* filename, unsigned char* inData, int w, int h, int samples_per_pixel, int depth)
{
    ...
    fclose(fp);
		...
		printf("(Output File %s) Picture Size %d (h) x %d (w): Total Pixel %d Sample: %d Depth: %d\n", filename,  h, w, (h)*(w), samples_per_pixel, depth);
}
